...
const getExtendedSites = async (
  sessionManager: SessionManager
): Promise<Result<Array<SiteWithExpiry>, RouteError>> =>
  chain3(
    sessionManager.getSessionUser(),
    ({ id }) => db.getAdminSites(id),
    async (sites) => {
      const fetchResultList = await Promise.all(
        sites.map(fetchSiteWithComments)
      )
      ...
      return fetchResultList.reduce((acc, withCommentsResult) => {
        ...
        return acc.map((sitesWithComments) =>
          sitesWithComments.concat(
            buildSite(withCommentsResult._unsafeUnwrap())
          )
        )
